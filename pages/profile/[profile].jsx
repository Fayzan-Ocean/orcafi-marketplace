...
const Edit_user = () => {
  ...
  const { loading, error, addUser,getUser } = useUserManagement();

	const username = useRef();
	const description = useRef('');
	const email = useRef('');
	const twitterUsername = useRef('');
	const instagramUsername = useRef('');
	const websiteUrl = useRef('');



	const uploadPhoto = async (file, _filename) => {
		try {
	...
	const handleFormSubmit = async (e)=>{
		e.preventDefault();
		setisLoading(true)
		const pName = Date.now() +'_'+ profilePhoto?.name ;
		const cName = Date.now() +'_'+ coverePhoto?.name ;
		...
		//const profileImageUrl = await uploadPhoto(profilePhoto, pName)
		//const coverImageUrl = await uploadPhoto(coverePhoto, cName)

		const profileImageUrl = await uploadImage(profilePhoto, pName.split('.').slice(0, -1).join('.'),'profileImages')
		const coverImageUrl = await uploadImage(coverePhoto, cName.split('.').slice(0, -1).join('.'),'coverImages')
		...
		let userData = {
		  ...
		  if (profilePhoto?.name) {
			userData.profileImage = pName;
		  }
		  
		  if (coverePhoto?.name) {
			userData.coverImage = cName;
		  }



		const updateUserData = await updateUserProfile(userData).then(()=>setisLoading(false));
	...
	const uploadImage = async (file,fname, folder) =>{
		...
		try {
			...
			formData.append('upload_preset','uonj6e9g')
			formData.append('folder',folder)
			formData.append('public_id',fname)

			const res = await fetch("https://api.cloudinary.com/v1_1/dnqloofr3/upload", {
			method: 'POST',
			body: formData,
			});	

			const uploadPimage =  await res.json()
...
const updateUserProfile = async (userData)=>{
	...
	const url = "/api/user/edit-user"; // Replace with the actual API endpoint URL
  ...
  try {
	...
	return (
		<div>
			<Meta title="Profile || Orcafi | NFT Marketplace " />
			<div className="pt-[5.5rem] lg:pt-24">
				{/* <!-- Banner --> */}
				<div className="relative">
					<img
						src={coverPreview ? coverPreview : getCldImageUrl({ src: "coverImages/"+user?.coverImage})}
						alt="banner"
				...
				<section className="dark:bg-jacarta-800 relative py-16">
					...
					<div className="container">
						<div className="mx-auto max-w-[48.125rem] md:flex">
							{/* <!-- Form --> */}
							<form className="mb-12 md:w-1/2 md:pr-8" onSubmit={handleFormSubmit}>
								<div className="mb-6">
								...
								<div className="mb-6">
									...
									<UserId
										...
										userId={address?.slice(0,-10)}
									/>
								</div>
								<button className="bg-accent shadow-accent-volume hover:bg-accent-dark rounded-full py-3 px-8 text-center font-semibold text-white transition-all" type='submit'>
									{isLoading? <div className='flex align-middle text-center justify-center'>
									<div className="inline-block h-5 w-5 animate-spin rounded-full border-4 border-solid border-current border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite] mr-2"
									role="status">
									<span
										className="!absolute !-m-px !h-px !w-px !overflow-hidden !whitespace-nowrap !border-0 !p-0 ![clip:rect(0,0,0,0)]"
										>Updating...
										</span>
									</div>Updating...</div> : <>Update Profile</>}
							...
							<div className="flex space-x-5 md:w-1/2 md:pl-8">
								<form className="shrink-0">
									<figure className="relative inline-block">
										<Image
											src={preview ? preview : getCldImageUrl({ src: "profileImages/"+user?.profileImage})}
