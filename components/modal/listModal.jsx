...
import  PolygonLogo from '../../public/images/chains/polygon.svg';
import { useBalance } from "wagmi";
import { useEffect } from "react";

const ListModal = () => {
  const { listModal,listOrderData } = useSelector((state) => state.counter);
  const dispatch = useDispatch();
  const [ETHAmount, setETHAmount] = useState(0.1);
  const{listNft} = useSwap()
  const {address} = useAccount()
  const {data} = useBalance({address:address})
  const [loadingList, setloadingList] = useState(false);
  const [usdMatic, setusdMatic] = useState(false);

  const handleEThAmount = async (e) => {
    e.preventDefault();
  ...
  const handleListNft = async (e)=>{
    ...
    const nft = {
        type: listOrderData?.payload?.metadata?.tokenType,
        tokenAddress: listOrderData?.payload?.metadata?.contract?.address,
        tokenId: listOrderData?.payload?.metadata?.tokenId,
    }
    const token = {
      type: "ERC20",
    ...
    console.log(token)
   const listNftData = await listNft(nft,token,address)
...
setloadingList(false)
  ...
  useEffect(()=>{
    ...
    const getMaticRate = async () =>{
      const data = await fetch("https://api.coincap.io/v2/assets/polygon")
      const usdMaticData = await data.json()
  
      setusdMatic(usdMaticData?.data?.priceUsd)
        ...
        <div className="modal-dialog max-w-2xl">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="placelistLabel">
            ...
            <div className="modal-body p-6">
              ...
              <div className="dark:border-jacarta-600 border-jacarta-100 relative mb-2 flex items-center overflow-hidden rounded-lg border">
                <div className="border-jacarta-100 bg-jacarta-50 flex flex-1 items-center self-stretch border-r px-2">
                  ...
                  <span>
                  <img src="/images/chains/polygon.svg" alt="React Logo" className="mr-1" width={20} height={20}/>
                  </span>
                ...
                <div className="bg-jacarta-50 border-jacarta-100 flex flex-1 justify-end self-stretch border-l dark:text-jacarta-700">
                  ...
                  parseFloat(parseFloat(usdMatic).toFixed(2)*parseFloat(ETHAmount).toFixed(2)).toPrecision(2)
            ...
            <div className="modal-footer">
              <div className="flex items-center justify-center space-x-4">
                <button
                  type="button"
                  className="bg-red  hover:bg-red rounded-full py-3 px-8 text-center font-semibold text-white transition-all"
                  onClick={handleListNft}
                >
                  {loadingList ? <> Loading..</> : <>List for Sale</>}
